local Libary =  loadstring(game:HttpGet("https://raw.githubusercontent.com/jbas68100-ux/Electronics-Hub...-3/refs/heads/main/Electronics%20Hub%20.txt"))()
workspace.FallenPartsDestroyHeight = -math.huge

local Window = Libary:MakeWindow({
    Title = "Swat Hub | Brookhaven RP 🏡",
    SubTitle = "By Lkzin & PhzinDev",
    LoadText = "Carregando Swat Hub | Brookhaven RP 🏡",
    Flags = "SwatHub"
})

Window:AddMinimizeButton({
    Button = {
        Image = 'rbxassetid://103742659711661',
...
local InfoTab = Window:MakeTab({ Title = "Info", Icon = "rbxassetid://103742659711661" })


local Translator = {
    traduzir = function(self, text)
        return text
    end
}

Tabs = {}
Tabs.M = InfoTab  -- FAZ Tabs.M APONTAR PARA A MESMA ABA Info


Tabs.M:AddDiscordInvite({
    Name = Translator:traduzir("Swat Hub | Community"),
    Description = Translator:traduzir("Junte-se à nossa comunidade discord para receber informações sobre a próxima atualização"),
    Logo = "rbxassetid://103742659711661",
    Invite = "https://discord.gg/JZ2uyPVEx"
})


InfoTab:AddSection({ "Informações do Script" })
InfoTab:AddParagraph({ "Owner / Developer:", "Lkzin & PhzinDev" })
InfoTab:AddParagraph({ "Collaboration:", " Rick/Shadow and Bazuka" })
InfoTab:AddParagraph({ "Leaked by:", "Bazuka." })
InfoTab:AddParagraph({ "You are using:", "Swat Hub Brookhaven RP 🏡" })
...
local function cleanupCouch()
    ...
    if char then
        local couch = char:FindFirstChild("Lkzin.Couch") or LocalPlayer.Backpack:FindFirstChild("Lkzin.Couch")
        if couch then
            couch:Destroy()
        end
    end
...
local ChildTab = Window:MakeTab({"Chield", "baby"})
...
scriptsTab:AddButton({
    ...
    Description = "Para o tornardo FE, remove o barco e ejeta o jogador",
    Callback = function()
        ...
        if not success then
            ...
            print("[Zyronis Hub] Tornado finalizado e veículos deletados.")
        end
    end
})
...
Tab:AddToggle({
    ...
    Callback = function(state)
        ...
        if selectedMethod == "Laptop" then
            ...
            if not laptopPath then warn("Laptop não encontrado.") return end
        ...
        elseif selectedMethod == "Telefone" then
            ...
            if not phonePath then warn("Telefone não encontrado.") return end
        ...
        elseif selectedMethod == "Bomba" then
            local Player = game.Players.LocalPlayer
            local Character = Player.Character or Player.CharacterAdded:Wait()
            local RootPart = Character:WaitForChild("HumanoidRootPart")
            local ReplicatedStorage = game:GetService("ReplicatedStorage")
